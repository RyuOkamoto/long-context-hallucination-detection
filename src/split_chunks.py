...
def transform_list_of_text_pairs(
    ...
    for i in range(len(texts1)):
        ...
        id2, mask2 = transform_single_text(
            texts2[i],
            tokenizer,
            chunk_size,
            stride,
    ...
    if split_sent:
        if pair_chunks:
            ...
            for i in range(len(input_ids)):
                ...
                for j in range(len(input_ids[i])):
                    for k in range(len(input_ids_2[i])):
                        paired_ids, paired_mask = pair_context_response_chunks(
                            input_ids[i][j], attention_mask[i][j], input_ids_2[i][k], attention_mask_2[i][k]
                        )
            ...
            tokens = {
                ...
                "response_input_ids": input_ids_2,
                "response_attention_mask": attention_mask_2,
            }
            return BatchEncoding(tokens)
...
def split_overlapping(
    ...
    if pad_original:
        ...
        result = [
            tensor[i * chunk_size_without_pad : (i + 1) * chunk_size_without_pad] for i in range(0, num_chunks - 1)
        ]
                ...
                for j in range(len(input_id_chunks[i])):
                    if torch.equal(mask_chunks[i][j], torch.Tensor([0])[0]):
                        input_id_chunks[i] = torch.cat(
                            [Tensor([0]), input_id_chunks[i][:j], Tensor([2]), input_id_chunks[i][j:]]
                        )
...
def check_split_parameters_consistency(
    chunk_size: int, stride: int, minimal_chunk_length: int, num_chunks, token_length, pad_last
) -> None:
    if pad_last:
        pass
    else:
...
def pair_context_response_chunks(input_ids, attention_mask, input_ids2, attention_mask2):
    if torch.equal(torch.sum(attention_mask), torch.Tensor([0])[0]) or torch.equal(
        torch.sum(attention_mask2), torch.Tensor([0])[0]
    ):
